...
function updateAwards(profileData) {
    const awards = document.getElementById('profile.awards')

    if (!profileData.awards || profileData.awards.length === 0) {
        awards.innerHTML = '<li>Nenhuma premiação informada</li>'
        return
    }

    awards.innerHTML = profileData.awards
...
function updateCourses(profileData) {
    ...
    if (!profileData.courses || profileData.courses.length === 0) {
        courses.innerHTML = '<li>Nenhum curso informado</li>'
        return
    }
